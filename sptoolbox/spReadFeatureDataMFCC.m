function [data, info] = spReadFeatureDataMFCC(name);
% [data, info] = readhtkdata(name)
% Read HTK format MFCC feature data
...
if fid == -1
  error('Unable to read %s', name)
else
  % Read HTK header
  info.Vectors = fread(fid,1,'uint32');         % number of vectors
  advance100Ns = fread(fid,1,'uint32');         % sample period in 100's ns
  ...
  % In HTK chapter 5 as of HTK 3.2.1
